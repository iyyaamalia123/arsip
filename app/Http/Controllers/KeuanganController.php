...
class KeuanganController extends Controller
{
    ...
    // FOLDER
    // Menampilkan tampilan awal menu keuangan
    public function index()
    {
        $menu = Menu::where('name', 'Keuangan')->first();
        $id_menu = $menu->id;
        $folders = Folder::where('id_menu', $id_menu)->get();
        return view('keuangan.index', compact('id_menu', 'folders'));
    }
    ...
    public function detail_folder($slug)
    {
        ...
        $filter = null;
        return view('keuangan.detail', compact('folder', 'files', 'years', 'filter'));
    }
    ...
    public function store_folder($id_menu, Request $request)
    {
        ...
        if ($cek_nama->isEmpty()) {
            ...
            $folder->url_folder = '/keuangan/'.Str::slug($request->name);
            $folder->is_main    = true;
            $folder->save();

            return redirect()->route('keuangan.index')->with('success', 'Berhasil Membuat Folder');
        }else{
            return redirect()->route('keuangan.index')->with('error', 'Nama folder sudah ada, gunakan nama lain!');
        }
    ...
    public function rename_folder($id, Request $request)
    {
        ...
        if ($cek_nama->isEmpty()) {
            ...
            $folder->url_folder = '/keuangan/'.Str::slug($request->name);
            $folder->save();
            return redirect()->route('kuangan.index')->with('success', 'Berhasil Merubah Nama Folder');
        }else{
            return redirect()->route('keuangan.index')->with('error', 'Nama folder sudah ada, gunakan nama lain!');
        }
    }
    ...
    public function store_file($id_folder, Request $request)
    {
        ...
        if ($cek_nama->isEmpty()) {
            ...
            return redirect('/keuangan/'.$folder->slug)->with('success', 'Berhasil Membuat File!');
        }else{
