...
class LainController extends Controller
{
    ...
    public function store_folder($id_menu, Request $request)
    {
        ...
        if ($cek_nama->isEmpty()) {
            ...
            $folder->url_folder = '/lain/'.Str::slug($request->name);
            $folder->is_main    = true;
            $folder->save();

            return redirect()->route('lain.index')->with('success', 'Berhasil Membuat Folder');
        }else{
            return redirect()->route('lain.index')->with('error', 'Nama folder sudah ada, gunakan nama lain!');
        }
    ...
    public function rename_folder($id, Request $request)
    {
        ...
        if ($cek_nama->isEmpty()) {
            ...
            $folder->url_folder = '/lain/'.Str::slug($request->name);
            $folder->save();
            return redirect()->route('lain.index')->with('success', 'Berhasil Merubah Nama Folder');
        }else{
            return redirect()->route('lain.index')->with('error', 'Nama folder sudah ada, gunakan nama lain!');
        }
    }
    ...
    public function store_file($id_folder, Request $request)
    {
        ...
        if ($cek_nama->isEmpty()) {
            ...
            return redirect('/lain/'.$folder->slug)->with('success', 'Berhasil Membuat File!');
        }else{
