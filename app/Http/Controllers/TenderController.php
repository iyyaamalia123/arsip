...
class TenderController extends Controller
{
    ...
    // FOLDER
    // Menampilkan tampilan awal menu tender
    public function index()
    {
        $menu = Menu::where('name', 'Tender')->first();
        $id_menu = $menu->id;
        $folders = Folder::where('id_menu', $id_menu)->get();
        return view('tender.index', compact('id_menu', 'folders'));
    }
    ...
    public function store_folder($id_menu, Request $request)
    {
        ...
        if ($cek_nama->isEmpty()) {
            ...
            $folder->url_folder = '/tender/'.Str::slug($request->name);
            $folder->is_main    = true;
            $folder->save();

            return redirect()->route('tender.index')->with('success', 'Berhasil Membuat Folder');
        }else{
            return redirect()->route('tender.index')->with('error', 'Nama folder sudah ada, gunakan nama lain!');
        }
    ...
    public function rename_folder($id, Request $request)
    {
        ...
        if ($cek_nama->isEmpty()) {
            ...
            $folder->url_folder = '/tender/'.Str::slug($request->name);
            $folder->save();
            return redirect()->route('tender.index')->with('success', 'Berhasil Merubah Nama Folder');
        }else{
            return redirect()->route('tender.index')->with('error', 'Nama folder sudah ada, gunakan nama lain!');
        }
    }
