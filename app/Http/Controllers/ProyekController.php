...
class ProyekController extends Controller
{
    ...
    // FOLDER
    // Menampilkan tampilan awal menu proyek
    public function index()
    {
        $menu = Menu::where('name', 'Proyek')->first();
        $id_menu = $menu->id;
        $folders = Folder::where('id_menu', $id_menu)->get();
        return view('proyek.index', compact('id_menu', 'folders'));
    }
    ...
    public function detail_folder($slug)
    {
        ...
        $filter = null;
        return view('proyek.detail', compact('folder', 'files', 'years', 'filter'));
    }
    ...
    public function store_folder($id_menu, Request $request)
    {
        ...
        if ($cek_nama->isEmpty()) {
            ...
            $folder->url_folder = '/proyek/'.Str::slug($request->name);
            $folder->is_main    = true;
            $folder->save();

            return redirect()->route('proyek.index')->with('success', 'Berhasil Membuat Folder');
        }else{
            return redirect()->route('proyek.index')->with('error', 'Nama folder sudah ada, gunakan nama lain!');
        }
    ...
    public function rename_folder($id, Request $request)
    {
        ...
        if ($cek_nama->isEmpty()) {
            ...
            $folder->url_folder = '/proyek/'.Str::slug($request->name);
            $folder->save();
            return redirect()->route('proyek.index')->with('success', 'Berhasil Merubah Nama Folder');
        }else{
            return redirect()->route('proyek.index')->with('error', 'Nama folder sudah ada, gunakan nama lain!');
        }
    }
    ...
    public function store_file($id_folder, Request $request)
    {
        ...
        if ($cek_nama->isEmpty()) {
            ...
            return redirect('/proyek/'.$folder->slug)->with('success', 'Berhasil Membuat File!');
        }else{
